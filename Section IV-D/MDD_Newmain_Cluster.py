...
def get_arr_AM(a,b,indicator):
    arr = []
    for i in range(a):
        for j in range(b):
            if indicator[i,j] <= 200:
                arr.append([i, j])
            else:
                continue
    return arr

def get_arr_DL(a,indicator):
    arr = []
    for i in range(a):
        index = np.where(indicator[i,:,0]>0)
        arr.append([i,index[0][0]])
    return arr
def get_arr_UL(a,indicator):
    arr = []
    for i in range(a):
        index = np.where(indicator[:,i,0]>0)
        for j in index[0]:
...
def build_graph(pre, com, DL_MUI, UL_Gain, UL_Inter, norm_pre, norm_com, AA, MM,
                norm_AP_dis, norm_MS_dis, norm_AM_dis, AP_dis, MS_dis, AM_dis, DLsub, ULsub):
    n = norm_pre.shape[0]
    ...
    x2 = []
    for i in range(n_AP):
        index = np.where(pre[i,:,0]>0)
        x1.append(np.squeeze(norm_pre[i,index,:]))
    for d in range(n_MS):
        index = np.where(com[:,d,0]>0)
        tmp = np.squeeze(norm_com[index,d,:]).reshape(-1,2)
    ...
    index_AP_AP = np.array(get_arr_AM(n_AP, n_AP, AP_dis)).transpose()
    index_MS_MS = np.array(get_arr_AM(n_MS, n_MS, MS_dis)).transpose()
    index_AP_MS = np.array(get_arr_DL(n_AP, pre)).transpose()
    index_MS_AP = np.array(get_arr_UL(n_MS, pre)).transpose()
    data['AP', 'IAI', 'AP'].edge_index = torch.tensor(index_AP_AP, dtype=torch.long)
    data['MS', 'IMI', 'MS'].edge_index = torch.tensor(index_MS_MS, dtype=torch.long)
    data['AP', 'DL', 'MS'].edge_index = torch.tensor(index_AP_MS, dtype=torch.long)
    data['MS', 'UL', 'AP'].edge_index = torch.tensor(index_MS_AP, dtype=torch.long)
    data['AP', 'IAI', 'AP'].edge_attr = torch.tensor(np.expand_dims(norm_AP_dis[index_AP_AP[0],index_AP_AP[1]], axis=1), dtype=torch.float)
    data['MS', 'IMI', 'MS'].edge_attr = torch.tensor(np.expand_dims(norm_MS_dis[index_MS_MS[0],index_MS_MS[1]], axis=1), dtype=torch.float)
    data['AP', 'DL', 'MS'].edge_attr = torch.tensor(np.expand_dims(norm_AM_dis[index_AP_MS[0],index_AP_MS[1]], axis=1), dtype=torch.float)
    norm_AM_dis_T = np.transpose(norm_AM_dis)
    data['MS', 'UL', 'AP'].edge_attr = torch.tensor(np.expand_dims(norm_AM_dis_T[index_MS_AP[0],index_MS_AP[1]], axis=1), dtype=torch.float)
    ...
    data.DL_MUI = torch.tensor(DL_MUI, dtype=torch.float)
    data.UL_Gain = torch.tensor(UL_Gain, dtype=torch.float)
    data.UL_Inter = torch.tensor(UL_Inter, dtype=torch.float)
    return data


def proc_data(pre, com, DL_MUI, UL_Gain, UL_Inter,
              norm_pre, norm_com, AA, MM, norm_AP_dis, norm_MS_dis, norm_AM_dis, AP_dis, MS_dis, AM_dis, config, train_layouts):
    #n = norm_pre.shape[0]
    n = train_layouts
    DLsub = config.DLsub
    ULsub = config.ULsub
    data_list = []
    for i in range(n):
        data = build_graph(pre[i, :, :, :], com[i, :, :, :], DL_MUI[i, :, :, :], UL_Gain[i, :, :],
                           UL_Inter[i, :, :, :], norm_pre[i, :, :, :], norm_com[i, :, :, :],
        AA[i, :, :], MM[i, :, :], norm_AP_dis[i, :, :], norm_MS_dis[i, :, :], norm_AM_dis[i, :, :], AP_dis[i, :, :],
                           MS_dis[i, :, :], AM_dis[i, :, :], DLsub, ULsub)
...
def MDD_loss(AP_pow, MS_pow, config, data):
    ...
    y_pre_mask1 = torch.reshape(data.y_pre, (-1,config.l_AP,data.y_pre.size()[1]))
    ...
    y_pre_mask = torch.reshape(y_pre_mask1[torch.where(y_pre_mask1>0)], (Batchs, config.l_AP, -1))
    y_pre_mask2 = torch.reshape(y_pre_mask1, (Batchs, config.l_AP, config.d_MS, -1))
    y_pre_index = torch.where(y_pre_mask2[:, :, :, 0] > 0)
    tool_index = config.d_MS * torch.arange(0,Batchs,device=device)
    tool_index = tool_index.expand(config.l_AP, Batchs).t().flatten()
    AP_MS_index = (y_pre_index[2]+tool_index)
    MS_DL_sum_mask = torch.sqrt(AP_mask) * y_pre_mask
    MS_DL_sum_mask = torch.reshape(MS_DL_sum_mask, (-1,config.DLsub))
    MS_DL_sum = scatter(MS_DL_sum_mask, AP_MS_index, dim=0, reduce='add')
    DL_signal = torch.pow(MS_DL_sum.reshape(Batchs,-1),2)
    ...
    DL_MUI_mask = torch.unsqueeze(AP_mask, axis=2) * data.DL_MUI.reshape((Batchs,config.l_AP,-1,AP_pow.size()[1]))
    DL_MUI_mask = torch.reshape(torch.sum(DL_MUI_mask, axis=1), (Batchs,-1))
    DL_NI = DL_NI + DL_MUI_mask
    ...
    UL_MUI_mask = torch.unsqueeze(MS_mask, axis=1) * data.UL_Inter.reshape((Batchs, config.d_MS, config.d_MS, -1))
    UL_MUI_mask = torch.sum(UL_MUI_mask, axis=2).reshape(-1,config.ULsub)
    temp = torch.squeeze(torch.sum(torch.unsqueeze(noisePlusinter_AP,2) * y_com_mask, 1, keepdim=True))
    UL_NI = torch.reshape(temp, MS_pow.size()) + UL_MUI_mask
    UL_Gain = torch.pow(data.UL_Gain,2)
    UL_signal = MS_pow * UL_Gain
...
# trainFile = 'C:\\MDD\\WorkData\\train_2412_10000_clu.mat'
# testFile = 'C:\\MDD\\WorkData\\test_2412_1000_clu.mat'
trainFile = 'C:\\MDD\\WorkData\\train_126_10000_clu.mat'
testFile = 'C:\\MDD\\WorkData\\test_126_1000_clu.mat'
...
train_DL_MUI, test_DL_MUI = train_data['DL_MUI'], test_data['DL_MUI']
train_UL_Gain, test_UL_Gain = train_data['UL_Gain'], test_data['UL_Gain']
train_UL_Inter, test_UL_Inter = train_data['UL_Inter'], test_data['UL_Inter']
...
train_data_list = proc_data(train_channelgain_precoder, train_channelgain_combiner,
                            train_DL_MUI, train_UL_Gain, train_UL_Inter,norm_train_pre,
                            norm_train_com, train_AA, train_MM, norm_train_AP_dis, norm_train_MS_dis,
                            norm_train_AM_dis, train_AP_dis, train_MS_dis,
                            train_AM_dis, train_config, train_layouts)
test_data_list = proc_data(test_channelgain_precoder, test_channelgain_combiner,
                           test_DL_MUI, test_UL_Gain, test_UL_Inter, norm_test_pre,
                           norm_test_com, test_AA, test_MM, norm_test_AP_dis, norm_test_MS_dis,
                           norm_test_AM_dis, test_AP_dis, test_MS_dis,
                           test_AM_dis, test_config, test_layouts)
...
class MDDGNN(torch.nn.Module):
    def __init__(self, num_layers, n_APch, n_MSch, config):
        ...
        for _ in range(num_layers):
            conv = MDDHeteroConv({'AP': DLsub, 'MS': ULsub}, data.metadata(),
            {
        ...
        self.LL1 = Seq(
            Linear(DLsub, 256, bias = True),
            LeakyReLU(),
            Linear(256, 512, bias=True),
            LeakyReLU(),
            Linear(512, DLsub, bias = True),
            ReLU()
        )
        self.LL2 = Seq(
            Linear(ULsub, 256, bias = True),
            LeakyReLU(),
...
# setup_seed(100)
model = MDDGNN(num_layers=8, n_APch=train_config.DLsub,
n_MSch=train_config.ULsub, config=train_config) ## in this case, we can increase the number of layers to achieve better scalability
...
model = best_model
torch.save(model.state_dict(), 'C:\\MDD\\ModelSave\\model_cluster_2412_8l_3')
testFile = 'C:\\MDD\\WorkData\\test_3015_1000_clu.mat'
test_data = scio.loadmat(testFile)
test_config = init_parameters()
test_config.l_AP = 30
test_config.d_MS = 15
...
test_AA, test_MM = test_data['AA'], test_data['MM']
test_AP_dis, test_MS_dis, test_AM_dis = test_data['AA_dis'], test_data['MM_dis'], test_data['AM_dis']
test_DL_MUI = test_data['DL_MUI']
test_UL_Gain = test_data['UL_Gain']
test_UL_Inter = test_data['UL_Inter']
...
test_data_list = proc_data(test_channelgain_precoder, test_channelgain_combiner,test_DL_MUI, test_UL_Gain, test_UL_Inter,
norm_test_pre, norm_test_com, test_AA, test_MM, norm_test_AP_dis, norm_test_MS_dis, norm_test_AM_dis, test_AP_dis, test_MS_dis,
                           test_AM_dis, test_config, test_layouts)
test_loader = DataLoader(test_data_list, batch_size=test_layouts, shuffle=False, num_workers=0)
loss_test, SE_test = test()
