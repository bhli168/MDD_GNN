function [p_initial,P_L_W,variPi_DL,variPi_UL,Psi_DL,Psi_UL] = MDD_ML_QT_initial_Cluster(Num_MS,Num_Sumsubcarrier,Num_DLsubcarrier,Num_ULsubcarrier,Num_AP,Chi_DL,Chi_UL,Power_AP_W,Power_MS_W,Omega,...
    Upsilon,DL_MUI,UL_inter,UL_gain,SI_cap_AP,SI_cap_MS,No,Beta_MS_MS,Beta_AP_AP,IAI_cap_AP,IMI_cap_MS)
...
expression P_L_m(Num_AP,Num_DLsubcarrier);
for ll = 1:Num_AP
    P_L_m(ll,:) = sum(p_temp(:,1:Num_DLsubcarrier,ll));
end
...
for d = 1:Num_MS
    for m = 1:Num_DLsubcarrier
        0 <= variPi_DL(d,m) <= Omega(:,d,m).' *  sqrt(squeeze(p_temp(d,m,:)));%% sqrt function ignores the nonzero items
        Psi_DL(d,m) >= SI_cap_MS*(sum(sum(p_temp(d,Num_DLsubcarrier+1:end,1)))) + ...
            IMI_cap_MS*sum(sum(Beta_MS_MS(d,:)*p_temp(:,Num_DLsubcarrier+1:end,1)))/Num_Sumsubcarrier + No +...
            DL_MUI(:,d,m).' * P_L_m(:,m);
...
for d = 1:Num_MS
    for m = 1:Num_ULsubcarrier
        0 <= variPi_UL(d,m) <= p_temp(d,Num_DLsubcarrier+m,1) * (UL_gain(d,m)^2);
        Psi_UL(d,m) >= SI_cap_AP*Upsilon(:,d,m).' * P_L_W...
            + (IAI_cap_AP/Num_Sumsubcarrier)*Upsilon(:,d,m).'* Beta_AP_AP * P_L_W...
            + No * sum(Upsilon(:,d,m)) + UL_inter(d,:,m) * squeeze(p_temp(:,Num_DLsubcarrier+m,1));
