function [R_sum,p,P_L_W,variPi_DL,variPi_UL,Psi_DL,Psi_UL] = MDD_ML_QT_Process_Cluster(Num_MS,Num_Sumsubcarrier,Num_DLsubcarrier,Num_ULsubcarrier,Num_AP,Chi_DL,Chi_UL,Power_AP_W,Power_MS_W,Omega,...
...
for d = 1:Num_MS
    for m = 1:Num_DLsubcarrier
        A(d,m) = Omega(:,d,m).' *  sqrt(squeeze(p_temp(d,m,:)));
        B(d,m) = SI_cap_MS*(sum(sum(p_temp(d,Num_DLsubcarrier+1:end,1)))) ...
            + IMI_cap_MS*sum(sum(Beta_MS_MS(d,:)*p_temp(:,Num_DLsubcarrier+1:end,1)))/Num_Sumsubcarrier...
            + No +  DL_MUI(:,d,m).' * P_L_m(:,m);
        R_max(d,m) = log(1 + 2 * z_DL(d,m) * A(d,m) - z_DL(d,m)^2 * B(d,m)) / Num_Sumsubcarrier;
    end
    for m = 1:Num_ULsubcarrier
        A(d,m + Num_DLsubcarrier) = p_temp(d,Num_DLsubcarrier+m,1) * (UL_gain(d,m)^2);
