function [] = MDD_ML_SE_main(SquareLength,Num_AP,Num_MS)
...
IMI_cap_MS = 10^(-4.2);
IAI_cap_AP = 10^(-7.2);
Chi_DL = 0.5; %% lower bound QoS requirements
Chi_UL = 0.1;
...
while ii<=999
    ...
    try
        ...
        [p,P_L_W,variPi_DL,variPi_UL,Psi_DL,Psi_UL] = MDD_ML_QT_initial(Num_MS,Num_Sumsubcarrier,Num_DLsubcarrier,Num_ULsubcarrier,Num_AP,Chi_DL,Chi_UL,Power_AP_W,...
            Power_MS_W,Omega,Upsilon,SI_cap_AP,SI_cap_MS,No,Beta_MS_MS,Beta_AP_AP,IAI_cap_AP,IMI_cap_MS);
        ...
        while (diff > 0.1 && iter <= 10)
            ...
            [R_sum,p,P_L_W,variPi_DL,variPi_UL,Psi_DL,Psi_UL] = MDD_ML_QT_Process(Num_MS,Num_Sumsubcarrier,Num_DLsubcarrier,Num_ULsubcarrier,Num_AP,Chi_DL,Chi_UL,Power_AP_W,Power_MS_W,Omega,...
        ...
        clear all
        load('tem.mat')
        delete('tem.mat') %% to avoid outage
    end
    
end

filename_2 = ['GNN_',num2str(Num_AP),num2str(Num_MS),'.txt']; 
