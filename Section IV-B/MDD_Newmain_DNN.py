...
def build_DNN(pre, com, norm_pre, norm_com, AA, MM, norm_AP_dis, norm_MS_dis, norm_AM_dis, DLsub, ULsub):
    ...
    dataAP=x1.reshape(576,1)
    dataMS = x2.reshape(288, 1)
    data= np.concatenate((dataAP,dataMS),axis=0)
    return data
...
class DNNnet(nn.Module):
    def __init__(self):
        ...
        self.module1 = nn.Sequential(nn.Linear(864, 1728),nn.ReLU())
        self.module2 = nn.Linear(1728, 2160)
        self.module3 = nn.Sequential(nn.Linear(2160, 1500), nn.ReLU())
        self.module4 = nn.Linear(1500, 1000)
        self.module5 = nn.Sequential(nn.Linear(1000, 588), nn.ReLU())
...
# torch.save(model.state_dict(), 'D:\\LBH\\MDD\\ModelSave\\model_246_DNN')

# save box plot
# model.load_state_dict(torch.load('D:\\LBH\\MDD\\ModelSave\\model_246_DNN'))
# model.eval()
# loss2, SE_test, SE_each_test = test()
# path = 'D:\\LBH\\MDD\\WorkData'
# fname1 = os.path.join(path,'DNN_246_test'+'.h5')
