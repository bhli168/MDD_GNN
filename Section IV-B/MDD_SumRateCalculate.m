function [R_sum_final,R_sum_MS] = MDD_SumRateCalculate(p,Omega,Upsilon,SI_cap_MS,SI_cap_AP,No,Num_AP,Num_MS,Num_DLsubcarrier,Num_ULsubcarrier,Num_Sumsubcarrier,...
...
for d = 1:Num_MS
    for m = 1:Num_DLsubcarrier
        ...
        B_dm(d,m) = SI_cap_MS*(sum(sum(p(d,Num_DLsubcarrier+1:end,1)))) + IMI_cap_MS*sum(sum(Beta_MS_MS(d,:)*p(:,Num_DLsubcarrier+1:end,1)))/Num_Sumsubcarrier + No;
        R_sum(d,m) = log(1 + A_dm(d,m)/ B_dm(d,m)) / Num_Sumsubcarrier;
    end
    for m = 1:Num_ULsubcarrier
        A_dmbar(d,m) = p(d,Num_DLsubcarrier+m,1) * (Num_AP^2);
