...
def MLP(channels, batch_norm=True):
    return Seq(*[
        Seq(Lin(channels[i - 1], channels[i]),BN(channels[i]), LeakyReLU())#,
...
class MDDSlimNet_AP(torch.nn.Module):
    def __init__(self, n_ch1):
        super(MDDSlimNet_AP, self).__init__()
        #self.slim_mlp = MDDSlimLinear(n_ch2, 128, max_dim)
        #self.mlp1 = MLP([n_ch2+sub1, 128, 256])
        self.mlp1 = MLP([384, 512])
        self.mlp2 = MLP([n_ch1+512, 2048])
        self.mlp2 = Seq(*[self.mlp2, Seq(Lin(2048, n_ch1), BN(n_ch1)),LeakyReLU()])
...
class MDDSlimNet_MS(torch.nn.Module):
    def __init__(self, n_ch1):
        super(MDDSlimNet_MS, self).__init__()
            # self.slim_mlp = MDDSlimLinear(n_ch2, 128, max_dim)
            # self.mlp1 = MLP([n_ch2+sub1, 128, 256])
        self.mlp1 = MLP([512, 1024])
        self.mlp2 = MLP([n_ch1 + 1024, 2048])
        self.mlp2 = Seq(*[self.mlp2, Seq(Lin(2048, n_ch1),BN(n_ch1)),LeakyReLU()])
        self.conv = IGConv(self.mlp1, self.mlp2)
